...
(function (global) {
  ...
  // data:, vbscript:) collapses to '#'. Control chars are stripped first so a
  // "java\tscript:" style filter bypass can't slip through; the current call
  // sites already exclude whitespace, but the function must be safe standalone.
  ...
  // Recursion cap for nested blockquotes/lists. A file that is just ">>>>…"
  // repeated a few thousand times would otherwise blow the call stack
  // (RangeError) and kill the viewer on a 5KB input. Past the cap, content
  // renders flat instead of nested.
  const MAX_DEPTH = 100;
  ...
  function renderBlocks(lines, depth) {
    ...
    while (i < lines.length) {
      ...
      if (/^\s{0,3}>/.test(line)) {
        ...
        if (depth < MAX_DEPTH) {
          html += `<blockquote>\n${renderBlocks(inner, depth + 1)}</blockquote>\n`;
        } else {
          html += `<p>${renderInline(inner.join('\n'))}</p>\n`;
        }
        continue;
      }
      ...
      if (/^\s*([-+*]|\d{1,9}[.)])\s+/.test(line)) {
        const consumed = renderList(lines, i, depth);
        html += consumed.html;
        i = consumed.next;
        continue;
      }
  ...
  // Past MAX_DEPTH, deeper items render flat at the current level.
  function renderList(lines, start, depth) {
    ...
    while (i < lines.length) {
      ...
      if (indent > baseIndent && depth < MAX_DEPTH) { // nested list under previous item
        const nested = renderList(lines, i, depth + 1);
